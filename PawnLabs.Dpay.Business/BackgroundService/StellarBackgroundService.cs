...
namespace PawnLabs.Dpay.Business.BackgroundService
{
    public class StellarBackgroundService : Microsoft.Extensions.Hosting.BackgroundService
    {
        ...
        private ISocketHelper _socketHelper;

        public StellarBackgroundService(IHttpClientFactory httpClientFactory, IOptions<StellarConfiguration> stellarConfiguration, 
            IPaymentService paymentService, IProductService productService, IConfigurationService configurationService, ISocketHelper socketHelper)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    foreach (var ev in events.EnumerateArray())
                    {
                        ...
                        var product = await _productService.GetByID(new Guid(productID));
                        ...
                        var walletAddress = (string)(await _configurationService.GetByType(new Configuration() { Email = product.Email, Type = EnumConfigurationType.WalletAddress })).Value;

                        if (!walletAddress.Equals(toAddress))
                            continue;

                        var paymentID = await _paymentService.Add(new Payment()
                        {
                            Email = product.Email,
                        ...
                        await _socketHelper.ConnectToHub(product.Email);
                        await _socketHelper.Connection.InvokeAsync("Payment", product.ID);
                        await _socketHelper.DisconnectFromHub();
